...
| **[Creative Futurology Pack](packs/creative-futurology-pack/)** | 12 | Turning signals of change into scenarios and scenarios into decisions — the focal question, the sourced signal scan, the trends, the uncertainty map, three or four constructed futures, the artifacts, the strategy wind tunnel, the roadmap, the signposts, the trend report, the workshop, and the leadership playback | `/plugin install creative-futurology-pack@polar-bear-skills` |
...
### Creative Futurology Pack — [`packs/creative-futurology-pack`](packs/creative-futurology-pack/)

Twelve skills for the work that starts when a client, or your own partners, want "something about the future" and nobody has said what decision it should change. The pack frames the question first, then scans for concrete, dated, sourced signals, clusters them into trends with an honest evidence grade, sorts the settled from the genuinely uncertain, and builds three or four constructed futures the team can walk into. From there it makes the futures tangible, runs the current strategy through each of them, turns what that teaches into moves with owners and dates, and keeps watching the signposts a quarter at a time. Claude scans and drafts; the leadership team does the arguing and the choosing. Real, dated signals are not optional and no scenario is ever a forecast — every skill in the pack will say so if asked which future will happen.

**Frame it and find the evidence**

| Skill | What it does | Say this to trigger it |
|-------|--------------|------------------------|
| [`focal-question-framer`](packs/creative-futurology-pack/skills/focal-question-framer/SKILL.md) | Turns "we should think about the future" into one focal question with a horizon, a scope and the decision it must inform, and fills the engagement brief every other skill reads | "we need a trends piece", "what should our offsite be about" |
| [`signal-scanner`](packs/creative-futurology-pack/skills/signal-scanner/SKILL.md) | Researches the web and logs concrete, dated, sourced signals, one card per signal with a link, across social, technological, economic, environmental and political territory — and merges the ones your team spotted. Never invents a signal | "find signals for our focal question", "we need evidence, not opinions" |
| [`trend-synthesizer`](packs/creative-futurology-pack/skills/trend-synthesizer/SKILL.md) | Clusters the signals into named trends with a direction, a maturity and an honest evidence grade. Three signals from two independent sources, or it stays a hunch | "what do these signals add up to", "cluster these" |

**Build the futures**

| Skill | What it does | Say this to trigger it |
|-------|--------------|------------------------|
| [`uncertainty-mapper`](packs/creative-futurology-pack/skills/uncertainty-mapper/SKILL.md) | Separates what is settled from what could go either way, sorts the drivers by impact and uncertainty, and proposes the two axes the scenarios stand on — recording the team's argument rather than replacing it | "which trends matter most", "pick the scenario axes" |
| [`scenario-builder`](packs/creative-futurology-pack/skills/scenario-builder/SKILL.md) | Three or four internally consistent constructed futures from the agreed axes, each with its logic, its timeline, who wins and who loses, and the first signs you would see. Labeled a constructed future, never a prediction | "write the scenarios", "build four worlds for 2030" |
| [`future-artifact-maker`](packs/creative-futurology-pack/skills/future-artifact-maker/SKILL.md) | Artifacts from inside each future — a headline, a product listing, a complaint letter, an annual report excerpt, a job ad — clearly marked fictional and built to make a room react rather than nod | "make the scenarios tangible", "write a headline from 2030" |

**Turn them into decisions**

| Skill | What it does | Say this to trigger it |
|-------|--------------|------------------------|
| [`strategy-wind-tunneler`](packs/creative-futurology-pack/skills/strategy-wind-tunneler/SKILL.md) | Runs your current strategy, or a set of options, through every scenario and grades each as holds, conditional or fragile, with the trigger that would force a change | "are we fine in all four worlds", "which of these options survives" |
| [`implications-roadmap-builder`](packs/creative-futurology-pack/skills/implications-roadmap-builder/SKILL.md) | No-regret moves now, options to keep open, big bets with triggers — each with an owner and a date. It does not choose the bet for you | "so what do we do", "turn the scenarios into actions" |
| [`signpost-tracker`](packs/creative-futurology-pack/skills/signpost-tracker/SKILL.md) | The early-warning indicators for each scenario, with thresholds and a review cadence, plus the quarterly re-scan that keeps the scenarios and the roadmap alive | "which future are we in now", "quarterly scenario review" |

**Take it to the room**

| Skill | What it does | Say this to trigger it |
|-------|--------------|------------------------|
| [`trend-report-writer`](packs/creative-futurology-pack/skills/trend-report-writer/SKILL.md) | The publishable trend report you send to clients: each trend as a tension with a question for leaders, sourced signals, implications and a provocation. Refuses a trend without sourced signals | "write our annual trends report", "draft the trends article" |
| [`scenario-workshop-runner`](packs/creative-futurology-pack/skills/scenario-workshop-runner/SKILL.md) | The half-day or full-day session where a team sorts the uncertainties, builds or tests the scenarios and claims the first moves: agenda, materials, facilitation prompts, capture. The room stays human | "plan the scenario workshop", "we're running the session next week" |
| [`leadership-playback-designer`](packs/creative-futurology-pack/skills/leadership-playback-designer/SKILL.md) | The playback that makes leadership care: the pre-read, the 45-minute arc, the one decision asked for, and the capture afterwards. It will not ship the playback as a document instead of a session | "present the scenarios to the board", "how do I get leadership to care" |

Shared files: [`templates/`](packs/creative-futurology-pack/templates/) ships `engagement-brief.md` (the context file every skill reads first — focal question, horizon, the decision it informs, scope, who is in the room — filled with focal-question-framer and kept current by whoever leads the engagement) and `signal-log.md` (the blank signal cards anyone on the team fills when they notice something, which signal-scanner checks and merges into the signal log).
...
│   ├── creative-futurology-pack/ # same shape; its shared files live in templates/
